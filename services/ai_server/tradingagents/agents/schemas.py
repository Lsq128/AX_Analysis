...
def render_trader_proposal(proposal: TraderProposal) -> str:
    ...
    """
    # Keep BUY/HOLD/SELL in English (industry convention + stop-signal parsers).
    # Field labels are Chinese-primary with English gloss.
    action = proposal.action.value.upper()
    parts = [
        f"**动作 (Action)**：{action}",
        "",
        f"**理由 (Reasoning)**：{proposal.reasoning}",
    ]
    if proposal.entry_price is not None:
        parts.extend(["", f"**参考入场 (Entry Price)**：{proposal.entry_price}"])
    if proposal.stop_loss is not None:
        parts.extend(["", f"**止损 (Stop Loss)**：{proposal.stop_loss}"])
    if proposal.position_sizing:
        parts.extend(["", f"**仓位 (Position Sizing)**：{proposal.position_sizing}"])
    parts.extend([
        "",
        # Trailing line must remain greppable for analyst stop-signal text.
        f"最终交易建议 (FINAL TRANSACTION PROPOSAL)：**{action}**",
    ])
    return "\n".join(parts)
